...
import { ProfileCreateResponse } from "../dataobjects/rest/response/profileCreate.response";
...
export async function createProfile(userId: string): Promise<ProfileCreateResponse> {
    try {
        const existingProfile = await profileData.getOneByUserId(userId);
    } catch (error: any) {
        if (error instanceof NotFoundError) {
            try {
                const user = await userData.getOneById(userId);

                const profile = new ProfileEntity();
                profile.userId = userId;
                
                const savedProfile = await profileData.save(profile);

                return { id: savedProfile.id!! }
            } catch (error: any) {
...
export async function updateByUserId(userId: string, requestBody: ProfileUpdateRequest) {
    ...
    if (requestBody.persona) {
        existingProfile.persona = {
            ...
            sex: requestBody.persona.sex ? requestBody.persona.sex : null,
